...
const SERVER_ZIP = 'https://github.com/Sahur01-arch/Voxel-beta/archive/refs/heads/main.zip';
const VERSION_URL = 'https://raw.githubusercontent.com/Sahur01-arch/Voxel-beta/main/package.json';
...
async function updateSettings({
	...
	return new Promise((resolve, reject) => {
		try {
			...
			if (apikey) {
				content = content.replace(/(global\.APIKeys\s*=\s*\{[\s\S]*?'https:\/\/api\.voxel\.biz\.id'\s*:\s*')[^']*(')/, `$1${apikey}$2`);
				if (global.APIKeys) global.APIKeys[global.APIs.voxel] = apikey;
			}
			if (botname) {
