...
function _drawBg(ctx,img,W,H){
    ...
    if(img){const sc=Math.max(W/img.width,H/img.height),dw=img.width*sc,dh=img.height*sc;tc.drawImage(img,(W-dw)/2,(H-dh)/2,dw,dh);}
        ...
        // Fallback saat tidak ada foto: gradient WA doodle-style (bukan flat solid)
        // biar tetap ada kedalaman/tekstur, jangan sampai kelihatan "kaku"/mati rata.
        const g=tc.createLinearGradient(0,0,W,H);
        g.addColorStop(0,'#1f2c24');
        g.addColorStop(0.5,'#141d19');
        g.addColorStop(1,'#0b1210');
        tc.fillStyle=g;tc.fillRect(0,0,W,H);
        // sedikit noise pattern lingkaran samar biar ada tekstur, meniru wallpaper WA
        tc.globalAlpha=0.05;
        for(let i=0;i<40;i++){
            tc.beginPath();
            tc.arc(Math.random()*W,Math.random()*H,Math.random()*60+20,0,Math.PI*2);
            tc.fillStyle='#ffffff';
            tc.fill();
        }
        tc.globalAlpha=1;
...
async function generateIQC(opts){
    ...
    const MNU_W=CW-2*LEFT_X, MNU_R=13*S, ITEM_H=48*S;
    const ITEMS=[
    ...
    const STATUS_H=44*S;
    const TOP_PAD=STATUS_H+60*S; // dikurangi dari 120*S -- versi lama kelihatan terlalu banyak ruang kosong di atas
    const GAP=8*S;
