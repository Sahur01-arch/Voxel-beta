...
async function requestHandler(config, useFallback = false) {
	try {
		const res = await axios(config)
		const htmlCheck = typeof res.data === 'string' ? res.data : JSON.stringify(res.data)
		if (!useFallback && (res.status >= 400 || htmlCheck.includes('cf-browser-verification') || htmlCheck.includes('Checking your browser'))) {
			throw new Error('Upstream protection detected')
		}
		return res
	} catch (err) {
		// Voxel adaptation: do not execute remote JavaScript or use a browser.
		// A lightweight native fetch fallback is enough for redirects/GET pages.
		if ((config.method || 'GET').toUpperCase() !== 'GET' || useFallback) throw err
